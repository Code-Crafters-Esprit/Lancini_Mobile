...
public class AfficherProduitForm extends Form {
    ...
    public void addElement(Produit produit) {
        ...
        try {
            placeholder = EncodedImage.create("/placeholder_image.png");
            if (produit.getImage() != null) {
                image = Image.createImage("/" + produit.getImage());
            } else {
                image = placeholder;
            }
...
//});
        ...
        Button updateButton = new Button("Buy");
...
//        }
            boolean success = ServiceProduit.getInstance().Commander(produit.getIdProduit());

            if (success) {
                Dialog.show("Order Placed", "Your order has been placed successfully.", "OK", null);
            } else {
                Dialog.show("Order Failed", "Failed to place your order. Please try again.", "OK", null);
            }
